...
// taken from: `https://github.com/libevm/hp2/blob/main/src/Deployer.sol`
...
function create(bytes memory bytecode, uint256 value)
...
function appendArg(bytes memory bytecode, address arg)
    pure
...
function deployProxy(
    ...
    bytes memory bc = vm.compile("huff/Proxy.huff").appendArg(impl).appendArg(
        owner
    );
    bc[immutableAddressOffset] = bytes1(
        0x20 + uint8(bc[immutableAddressOffset])
    );
    return create(bc, 0);
}
